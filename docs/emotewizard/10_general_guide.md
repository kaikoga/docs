...
Emote Wizardでは`ew:EmoteWizardRoot`やアバター以下の各所に設定用のコンポーネントを配置して、
アバタービルド時に各所の情報を収集して設定を統合することで必要なアバターセットアップを生成します。

この時に使う設定用のコンポーネントをData Source（または、単にSource）と呼びます。

:::note[NOTE]
Sourceは`ew:EmoteWizardRoot`の子孫に加えて、アバターの子孫に配置したものが認識されます。
Modular Avatarを利用する場合、衣装プレハブのPrefab Variantを作った上で追加でSourceの設定を行うことが可能です。
:::

なお、Data Sourceのうち、 `ew:EmoteItemSource` が機能するには追加のコンポーネントが必要です。
具体的には、 `ew:EmoteSequenceSource` または `ew:GenericEmoteSequenceSource` と一緒に働きます。
...
また、 `ew:EmoteWizardRoot` 自体の追加設定を行う補助的なコンポーネントもあります。  
Configと呼ばれるコンポーネントがそれです。

:::info[INFO]
Configは必要に応じて `ew:EmoteWizardRoot` と同じオブジェクトに追加されます。
...
- Data Sourceを作りたくなったら、まずは `ew:EmoteWizardDataSourceFactory`からWizardを作る
