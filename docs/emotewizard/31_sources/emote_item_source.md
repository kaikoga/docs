...
アニメーションの内容は `ew:EmoteSequenceSource` または `ew:GenericEmoteSequenceSource` で設定します。

- `ew:EmoteSequenceSource` は詳細な設定が可能です。
- `ew:GenericEmoteSequenceSource` は作成が簡単です。
...
- `優先度`: 同じグループに属する複数のアニメーションが条件を満たした場合、優先度の小さいアニメーションが選択されます。
  グループは `ew:EmoteSequenceSource` または `ew:GenericEmoteSequenceSource` で設定します。
