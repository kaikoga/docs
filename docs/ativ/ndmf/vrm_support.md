...
- NDMF
- UniVRM
- Ablet （NDMFからVRMファイルの出力を行う場合に内部的に利用します）

:::info[INFO]
Abletがない場合もNDMFのプラットフォームは追加されますが、この場合はNDMFからVRMファイルを出力することができません。
:::
...
- Manual Bakeする（必要に応じ、生成結果をNDMF ConsoleまたはAbletコンソールからエクスポートする）
...
NDMFまたはAbletのUIからエクスポートを行った場合はテクスチャのインポート設定の変更は行われません。
このため、出力されるテクスチャの画質に影響がある場合があります。
:::

# 既知のNDMF Apply on Playの制限
