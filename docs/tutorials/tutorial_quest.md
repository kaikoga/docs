...
:::note[NOTE]
Unityのプロジェクトを分ける必要はありませんが、VRChat PCにアップロードするためのアバターとは別のオブジェクトとして編集することをお勧めします。
...
QuestReplacerを利用することで、マテリアルのみを手動変換します。
...
QuestReplacerのDatabaseが必要に応じて自動生成されるので、クリックして参照し、必要に応じて `Generated Directory` 以下の設定を確認してください。
...
:::info[INFO]
マテリアルを変更するため、アバターの見た目がおかしくなることがあります。

その場合、生成されたマテリアルを調整して対応してください。
:::
