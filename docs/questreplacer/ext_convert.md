...
`マテリアル生成モード` | `Material Generation Mode` の `Ext Convert ...` シリーズを選択している場合、マテリアル生成の際に、拡張コンバートを行います。
拡張コンバートでは通常のマテリアル生成と比べて、より見た目の近いマテリアルに変換されやすくなります。

:::warning[WARNING]
拡張コンバートは外部ライブラリの非公開APIを利用していることがあるため、更新に伴い動かなくなることが予想されます。

`マテリアル生成モード` | `Material Generation Mode` の `Generate ...` シリーズを使うことで、拡張コンバート機能を無効化できます。
