...
ロード操作を行うには、 `データベース` | `Database` のすぐ下にある `読み込み` | `Load` ボタンを押してください。

:::info[INFO]
`収集` | `Collect` 操作ではアバターに含まれる全てのマテリアル等を収集しますが、データベースの置き換えパターンは参照しません。

`読み込み` | `Load` 操作ではデータベースが置き換えパターンを知っているマテリアルについて、置き換えパターンをアバターに追加します。
:::
...
セーブ操作を行うには、 `データベース` | `Database` のすぐ下にある `書き出し` | `Save` ボタンを押してください。
...
- `コンポーネントフィルタ` | `Component Filters`
  - アセットを収集する対象のコンポーネントの型名がここに記録されます。
- `生成先ディレクトリ` | `Generated Directory`
  - `Assets/` で始まるディレクトリをここに指定することで、生成されたマテリアルを特定のディレクトリに保存できます。
  - ここを空欄にした場合、生成されたマテリアルは元のマテリアルと同じディレクトリに生成されます。
- `生成ファイルプレフィクス` | `Generated File Prefix`
  - 生成されたマテリアルのファイル名の最初につける接頭辞を設定します。
- `生成ファイルサフィックス` | `Generated File Suffix`
  - 生成されたマテリアルのファイル名の最後につける接尾辞を設定します。

:::tip[TIPS]
Unityのマテリアルアセット選択UIにはマテリアルのファイル名しか表示されないため、 `生成ファイルプレフィクス` | `Generated File Prefix` と `生成ファイルサフィックス` | `Generated File Suffix` のどちらかを設定しておくことをお勧めします。
:::
